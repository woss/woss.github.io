...
<div class="flex items-end gap-3">
  <textarea
    class="flex-1 font-body text-base text-on-surface bg-surface-container-high border border-[rgba(255,255,255,0.08)] rounded-lg px-4 py-3 min-h-[44px] max-h-[120px] resize-none outline-none transition-colors duration-150 leading-normal placeholder:text-on-surface-variant focus:border-primary focus:shadow-[0_0_0_1px_var(--color-primary),0_0_16px_rgba(0,255,136,0.12)] disabled:opacity-50 disabled:cursor-not-allowed"
  ...
  <div class="flex items-center gap-2 shrink-0 self-end">
    ...
    <button
      class="flex items-center justify-center w-[44px] h-[44px] border-0 rounded-lg bg-primary text-surface cursor-pointer transition-all duration-150 shrink-0 hover:shadow-[0_0_20px_rgba(0,255,136,0.15)] hover:scale-105 active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed disabled:scale-100 disabled:shadow-none"
      disabled={!canSend}
      onclick={() => onsend(messageText)}
      aria-label="Send message"
    >
      {#if isLoading}
        <span class="w-[10px] h-[10px] rounded-full bg-surface shadow-[0_0_6px_var(--color-surface)] animate-pulse-send" aria-hidden="true"></span>
      {:else}
