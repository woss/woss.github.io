        ...
        >
          <a href={resolve(`/posts/${post.slug}`)} class="block" tabindex="-1">
            {#if post.headerImage}
              ...
              {/if}
            {:else}
              <img src={`/api/og/${post.slug}.png`} alt={post.title} class="w-full aspect-2/1 object-cover" />
            {/if}
          </a>
          <a href={resolve(`/posts/${post.slug}`)} class="flex flex-col p-6 no-underline text-inherit gap-3 focus-visible:outline-2 focus-visible:outline-primary focus-visible:-outline-offset-4 focus-visible:rounded-lg" aria-label="Read: {post.title}">
            {#if post.tags.length > 0}
              <div class="flex flex-wrap gap-2 mb-1">
                {#each post.tags as tag (tag)}
                  <span class="font-mono text-xs font-medium text-secondary bg-[color-mix(in_srgb,var(--color-secondary)_10%,transparent)] px-2 py-0.5 rounded-sm tracking-[0.02em] uppercase">{tag}</span>
                {/each}
              </div>
            {/if}

            <h2 class="font-heading text-lg md:text-xl font-bold text-on-surface leading-tight m-0 tracking-[-0.01em]">{post.title}</h2>

            <p class="text-sm text-on-surface-variant leading-normal m-0 line-clamp-2">{post.excerpt}</p>

            {#if post.date}
              <time datetime={post.date} class="font-mono text-xs text-outline mt-auto pt-2 border-t border-[rgba(255,255,255,0.08)]">{formatDate(post.date)}</time>
            {/if}
          </a>
        </article>
      {/each}
