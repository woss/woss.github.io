...
{:else if data?.hero}
<!-- Hero content (NO bg image/overlay — those are rendered above) -->
<div class="relative z-[2] flex flex-col items-center justify-center min-h-[calc(100vh-var(--nav-height)-3rem)] gap-8 px-8 max-md:px-4 max-md:gap-6">
  <div class="w-full flex flex-col items-center gap-6 pt-16 pb-16">
    <!-- Hero text group -->
    <div class="w-full max-w-[800px] flex flex-col items-center gap-3 text-center">
      <h1 class="font-heading text-4xl md:text-5xl lg:text-6xl text-on-surface m-0 leading-tight">
        {data.hero.title}
      </h1>
      <p class="font-body text-base md:text-lg text-on-surface-variant m-0">
        {data.hero.description}
      </p>
      <a href={resolve('/posts/new-woss-io')} class="font-body text-sm text-primary/70 hover:text-primary transition-colors duration-150 no-underline">
  ...
  <!-- Footer tagline -->
  <div class="flex flex-col items-center gap-4 w-full max-w-120 pb-8">
    <div class="w-full h-px bg-[linear-gradient(90deg,transparent,rgba(255,255,255,0.08)_20%,rgba(255,255,255,0.08)_80%,transparent)]" aria-hidden="true"></div>
    <p class="font-body text-sm text-on-surface-variant text-center m-0">Ask about my work, career, skills, and experience.</p>
  </div>
