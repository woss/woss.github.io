...
type DelegationTerminalStatus = Extract<DelegationStatus, 'complete' | 'error' | 'cancelled' | 'timeout'>;
...
export class DelegationManager {
  ...
  constructor(client: OpencodeClient, baseDir: string, log: Logger, options: DelegationManagerOptions = {}) {
    this.client = client;
  ...
  private resetParentAllCompleteNotificationCycle(parentSessionID: string): void {
    ...
    state.allCompleteCycleToken = this.buildAllCompleteCycleToken(parentSessionID, state.allCompleteCycle);
  ...
  private areCycleTerminalNotificationsComplete(parentSessionID: string, cycleToken: string): boolean {
  ...
  private buildAllCompleteNotification(parentSessionID: string, cycle: number, cycleToken: string): string {
  ...
  private async waitForPersistedArtifact(filePath: string, maxWaitMs: number): Promise<string | null> {
  ...
  private async finalizeDelegation(
    ...
    if (resolvedResult.trim().length > 0) {
      const metadata = await this.metadataGenerator(this.client, resolvedResult, delegation.sessionID, (msg) =>
        this.debugLog(msg),
      );
  ...
  private async handleTimeout(delegationId: string): Promise<void> {
    ...
    await this.finalizeDelegation(delegation.id, 'timeout', `Delegation timed out after ${this.maxRunTimeMs / 1000}s`);
  }
...
`;
  ...
  async readOutput(sessionID: string, id: string): Promise<string> {
    ...
    const fallbackFilePath = path.join(await this.getDelegationsDir(sessionID), `${normalizedId}.md`);
    ...
    if (!delegation) {
      throw new Error(`Delegation "${normalizedId}" not found.\n\nUse delegation_list() to see available delegations.`);
    }
    ...
    for (const delegation of this.delegations.values()) {
      ...
      results.push({
        ...
        description:
          ...
          (delegation.status === 'running' || delegation.status === 'registered' ? '(running)' : '(no description)'),
        agent: delegation.agent,
...
export function formatDelegationContext(
  ...
  if (running.length > 0) {
    ...
    sections.push('> **Note:** You WILL be notified via `<task-notification>` when delegations complete.');
  ...
  if (unreadCompleted.length > 0) {
    ...
    for (const d of unreadCompleted) {
      const statusEmoji =
        d.status === 'complete' ? '✅' : d.status === 'error' ? '❌' : d.status === 'timeout' ? '⏱️' : '🚫';
